...
if (mysqli_num_rows($result) > 0) {
		echo "ID	 	DateTime		TempOutCur		TempOutCur		PressOutCur		WindOutCur    WindDirOutCur<br>";
		while($row = mysqli_fetch_assoc($result)) {
		   echo $row["ID"] . "	\t\t\t\t\t	" . $row["DateTime"] . "	\t\t\t\t\t\t	" . $row["TempOutCur"] . "	\t\t	" . $row["TempOutCur"] ."	\t\t	" . $row["PressOutCur"] ."	\t\t	" . $row["WindOutCur"] ." \t\t	" . $row["WindDirOutCur"] . " <br>";
		}
	 }
 else {
		echo "0 results";
